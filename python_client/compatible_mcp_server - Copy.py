#!/usr/bin/env python3
"""
Version-compatible MCP server for ShiftWork API
"""
...
class CompatibleShiftWorkServer:
    def __init__(self):
            ...
            async with stdio_server() as (read_stream, write_stream):
                await self.server.run(
                    ...
                    InitializationOptions(
                        ...
                        capabilities=self._get_capabilities_safely()
                    )
                )
...
async def main():
    server = CompatibleShiftWorkServer()
    await server.run()
